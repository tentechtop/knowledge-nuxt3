        ...
        <div class="message-item" v-for="item of messageList.filter((v) => v.role !== 'system')">
          ...
          <div class="user-message-content" v-if="item.role !== 'user'" :class="item.role === 'user' ? 'user-message':'service-message'">
            ...
            <div class="no-user-message-item"
                 v-if="item.content"
                 v-html="getHtml({content : item.content})"
            ></div>
          </div>
        </div>
      </div>
...
function isStringAllSpaces({inputString}: { inputString: any }) {
  // 使用trim()方法去掉字符串两端的空格，然后检查结果是否为空字符串
  return inputString.trim() === '';
}

function getAvailableButton(){
  if (isStringAllSpaces({inputString: messageContent.value}) ||messageContent.value.length===0 || isTalking.value===true ){
...
let apiKey = "sk-KjXMQ9j99ARH44sk7e55C899823249089f6cF67272C57b68";
...
function getHtml({content}: { content: any }){
...
function isKeywordHit(content, keywords: any[]) {
